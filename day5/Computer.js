...
class Computer {
    ...
    run() {
        ...
        while (address < length) {
            ...
            if ([TYPE_ADD, TYPE_MULTIPLY].includes(opCode)) {
                ...
                // Next code should be 4 positions away
                address += 4;
            } else if ([TYPE_GET, TYPE_SET].includes(opCode)) {
                let paramA = state[address[1]];
                // If using positional mode...
                let valAtPosition = state[paramA];
                if (opCode === TYPE_GET) {
                    // output requested parameter
                    console.log(paramA);
                } else if (opCode === TYPE_SET) {
                    // in positional mode, set value at position TO the param
                    state[paramA] = paramA;
                }
                // Next code should be 2 positions away
                address += 2;
            }
        }
        return this;
